...
`);gradePoses=t[0].split(",").map(e=>parseInt(e)),t.slice(1).forEach(e=>{vocabularies.push(e.split("	"))}),loadWorkers()})}function loadSiminymWorker(){let e=localStorage.getItem("rensole-ja");if(!e){const t=document.getElementById("grade");e=t.options[t.selectedIndex].value}const t={from:"jsonconfig",configUrl:`/siminym-ja/db/${e}/config.json`};return createDbWorker([t],"/rensole-ja/sql.js-httpvfs/sqlite.worker.js","/rensole-ja/sql.js-httpvfs/sql-wasm.wasm")}function loadRensoWorker(){const e={from:"jsonconfig",configUrl:"/rensole-ja/db/config.json"};return createDbWorker([e],"/rensole-ja/sql.js-httpvfs/sqlite.worker.js","/rensole-ja/sql.js-httpvfs/sql-wasm.wasm")}function loadProblemVectors(){const e=[getSiminyms(answer),getWordVector(answer)];return Promise.all(e).then(e=>{mostSimilars=e[0],answerVector=e[1],document.getElementById("searchText").focus(),document.getElementById("loading").classList.add("d-none")})}async function changeProblem(){document.getElementById("loading").classList.remove("d-none"),document.getElementById("answer").classList.add("d-none"),replyCount=0;const e=getRandomInt(0,problems.length);answer=problems[e][0],pronounce=problems[e][1].split("");const t=document.getElementById("renso");for(;t.firstChild;)t.firstChild.remove();await loadProblemVectors()}async function loadWorkers(){const e=document.getElementById("grade"),t=gradePoses[e.selectedIndex];problems=vocabularies.slice(0,t);const n=[loadSiminymWorker(),loadRensoWorker()];await Promise.all(n).then(e=>{siminymWorker=e[0],rensoleWorker=e[1],changeProblem(),document.getElementById("renso").classList.remove("d-none")})}function changeGrade(){const e=document.getElementById("grade"),t=e.options[e.selectedIndex].value;localStorage.setItem("rensole-ja",t),location.reload()}const jkat=new Kanji(JKAT),vocabularies=[];let gradePoses=[],problems=[],replyCount=0,mostSimilars,answerVector,answer,pronounce,holedAnswer,holedPronounce,rensoleWorker,siminymWorker;loadConfig(),loadGrade(),loadProblems(),document.addEventListener("keydown",e=>{e.key=="Enter"&&search()}),document.getElementById("toggleDarkMode").onclick=toggleDarkMode,document.getElementById("search").onclick=search,document.getElementById("restart").onclick=changeProblem,document.getElementById("grade").onchange=changeGrade,document.addEventListener("pointerdown",unlockAudio,{once:!0}),document.addEventListener("keydown",unlockAudio,{once:!0})